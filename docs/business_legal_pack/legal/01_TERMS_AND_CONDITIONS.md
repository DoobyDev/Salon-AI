# Terms and Conditions

Draft status:
- Working draft pending solicitor review before publication or reliance as final legal terms.

Effective date: TBD  
Operator: Stuart Hall ("Company", "we", "us")

## 1. Scope

These Terms govern use of:
- Website and app services branded as Ask Lexi | AI Salon Receptionist.
- Subscriber dashboards and customer-facing booking/reception features.
- Related API, desktop, and mobile interfaces.

## 2. Account Types

- `Admin`: platform-level management.
- `Subscriber`: hair salon, barbershop, or beauty salon business account.
- `Customer`: end-user booking/search account.

You are responsible for account security and all activity under your credentials.

## 3. Service Description

The platform provides:
- Business discovery and search.
- Booking slot visibility and booking management.
- AI receptionist chat assistance.
- Role-based dashboards for customers, subscribers, and admins.
- Reminder settings, scheduled reminders, and notification status visibility.
- Dashboard analytics and accounting exports.
- Web, desktop, mobile-wrapper, and installable PWA shell access paths.

No guarantee is made that AI output is accurate, complete, or suitable for legal, financial, or medical decisions.

## 4. Eligibility

You must be at least 18 and capable of forming a binding contract.

## 5. Acceptable Use

You must not:
- Use the service for unlawful, abusive, fraudulent, or misleading activities.
- Attempt unauthorized access, scraping beyond permitted use, or service disruption.
- Upload malicious code or violate third-party rights.

## 6. Subscriber Obligations

Subscribers must:
- Maintain accurate business information.
- Use lawful marketing and communications practices.
- Obtain all required customer consents.
- Comply with applicable tax, consumer, employment, and data laws.

## 7. Fees and Billing

Billing terms are set in `05_SUBSCRIPTION_AND_BILLING_POLICY.md`.
Subscriber billing is currently managed through Stripe and PayPal subscription flows exposed through the platform dashboard.

## 8. Data and Privacy

Privacy and data handling are described in:
- `02_PRIVACY_POLICY_UK_GDPR.md`
- `08_DATA_PROCESSING_ADDENDUM_DPA.md`

## 9. Intellectual Property

All platform code, branding, and content (excluding user content) remain Company property.

## 10. Availability and Changes

We may modify or discontinue features, and may schedule maintenance windows.
We do not guarantee uninterrupted availability.

## 11. Suspension and Termination

We may suspend/terminate for:
- Breach of these Terms.
- Fraud/security risk.
- Legal or regulatory requirement.

## 12. Disclaimers

Service is provided "as is" and "as available." To the fullest extent permitted by law, warranties are disclaimed.

## 13. Limitation of Liability

To the fullest extent permitted by law, Company is not liable for indirect, incidental, special, or consequential damages.

Company aggregate liability for claims arising in a 12-month period is limited to fees paid in that period.

## 14. Indemnity

You agree to indemnify Company from claims arising from your misuse, unlawful content, or policy breach.

## 15. Governing Law and Jurisdiction

These Terms are governed by the laws of England and Wales, with courts located in [PLACEHOLDER: VENUE].

## 16. Contact

stuhall44@gmail.com  
10 Wilson House, Denton Drive, Northwich, Cheshire, CW97XJ
